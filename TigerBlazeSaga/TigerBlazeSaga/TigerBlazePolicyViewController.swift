//
//  TigerBlazePolicyViewController.swift
//  TigerBlazeSaga
//
...
class TigerBlazePolicyViewController: UIViewController, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate {
    ...
    var privacyData: [Any]?
    @objc var url: String?
    let TigerBlazePrivacyPolicyUrl = "https://www.termsfeed.com/live/8ad2e7e9-4a21-4e5f-986a-5220b3a68d8a"
    ...
    */
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.privacyData = UserDefaults.standard.array(forKey: UIViewController.tigerBlazeGetUserDefaultKey())
        initSubViews()
        initNavView()
        initWebView()
        tigerBlazeStartLoadWebView()
    }
    ...
    private func tigerBlazeStartLoadWebView() {
    ...
    private func tigerBlazeReloadWebViewData(_ adurl: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let storyboard = self.storyboard,
               let adView = storyboard.instantiateViewController(withIdentifier: "TigerBlazePolicyViewController") as? TigerBlazePolicyViewController {
                adView.url = adurl
           ...
           let trackMessage = message.body as? [String: Any] {
            ...
            if let ty = confData[18] as? Int, ty == 1 {
                    ...
                    do {
                        if let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            if tName != (confData[8] as? String) {
                                tigerBlazeSendEvent(tName, values: jsonObject)
                                return
                            }
                            ...
                            if let adId = jsonObject["url"] as? String, !adId.isEmpty {
                                tigerBlazeReloadWebViewData(adId)
                            }
                        }
                    } catch {
                        tigerBlazeSendEvent(tName, values: [tName: data])
                    }
                } else {
                    tigerBlazeSendEvent(tName, values: [tName: tData])
                }
            } else if let ty = confData[18] as? Int, ty == 2 {
                tigerBlazeAfSendEvents(tName, paramsStr: tData)
            } else {
                if tName == confData[28] as? String {
                    ...
                    tigerBlazeAfSendEvent(withName: tName, value: tData)
                }
            }
            
        } else if name == (confData[19] as? String) {
            if let messageBody = message.body as? String,
               let dic = tigerBlazeJsonToDic(withJsonString: messageBody) as? [String: Any],
               let evName = dic["funcName"] as? String,
               let evParams = dic["params"] as? String {
                
                if evName == (confData[20] as? String) {
                    if let uDic = tigerBlazeJsonToDic(withJsonString: evParams) as? [String: Any],
                    ...
                    tigerBlazeSendEvents(withParams: evParams)
                }
            }
        }
    }
